...
import { n as QuoteType, r as Tokenizer, t as Parser } from "./Parser-BccsGLt7.js";
...
*/
const getCodePoint = typeof String.prototype.codePointAt === "function" ? (input, index) => input.codePointAt(index) : (c, index) => (c.charCodeAt(index) & 64512) === 55296 ? (c.charCodeAt(index) - 55296) * 1024 + c.charCodeAt(index + 1) - 56320 + 65536 : c.charCodeAt(index);
/**
...
//#endregion
//#region ../../node_modules/.pnpm/dom-serializer@3.0.0/node_modules/dom-serializer/dist/foreign-names.js
/**
...
//#endregion
//#region ../../node_modules/.pnpm/dom-serializer@3.0.0/node_modules/dom-serializer/dist/index.js
/**
...
function formatAttributes(attributes, options) {
	...
	return Object.keys(attributes).map((key) => {
		...
		if (options.xmlMode === "foreign") key = attributeNames.get(key) ?? key;
		if (!(options.emptyAttrs || options.xmlMode) && normalizedValue === "") return key;
...
function renderTag(element, options) {
	if (options.xmlMode === "foreign") {
		...
		if (element.parent && foreignModeIntegrationPoints.has(element.parent.name)) options = {
			...options,
			xmlMode: false
		};
	}
	if (!options.xmlMode && foreignElements.has(element.name)) options = {
		...options,
		xmlMode: "foreign"
	};
	...
	if (element.children.length === 0 && (options.xmlMode ? options.selfClosingTags !== false : options.selfClosingTags && singleTag.has(element.name))) {
		if (!options.xmlMode) tag += " ";
		tag += "/>";
	} else {
		tag += ">";
		if (element.children.length > 0) tag += render(element.children, options);
		if (!!options.xmlMode || !singleTag.has(element.name)) tag += `</${element.name}>`;
	}
	return tag;
}
...
function renderText(element, options) {
	...
	if ((options.encodeEntities ?? options.decodeEntities) !== false && !(!options.xmlMode && element.parent && unencodedElements.has(element.parent.name))) data = !!options.xmlMode || options.encodeEntities !== "utf8" ? encodeXML(data) : escapeText(data);
	return data;
}
...
//#endregion
export { DomHandler as DefaultHandler, DomHandler, dist_exports as DomUtils, dist_exports$1 as ElementType, Parser, QuoteType, Tokenizer, createDocumentStream, getFeed, parseDocument, parseFeed };
