...
const ComplexMappingChars2String = buildMapping("_c");
...
function createEscapeMapping(customMap) {
	if (!customMap) return DEFAULT_ESCAPE_MAPPING;
	if (customMap === DEFAULT_ESCAPE_MAPPING || customMap === COMPLEX_ESCAPE_MAPPING) return customMap;
...
function buildInverseMapping(mapping) {
	...
	return {
		inverse,
		tokens: Array.from(tokens).sort((a, b) => b.length - a.length)
	};
}
...
const DEFAULT_ESCAPE_TABLE = (() => {
	const table = Array.from({ length: 128 });
	for (const [char, token] of Object.entries(DEFAULT_ESCAPE_MAPPING)) table[char.codePointAt(0)] = token;
	return table;
})();
function readCodePoint(str, index) {
	const first = str.charCodeAt(index);
	if (first < HIGH_SURROGATE_MIN || first > HIGH_SURROGATE_MAX || index + 1 >= str.length) return {
		codePoint: first,
		size: 1
	};
	const second = str.charCodeAt(index + 1);
	if (second < LOW_SURROGATE_MIN || second > LOW_SURROGATE_MAX) return {
		codePoint: first,
		size: 1
	};
	return {
...
function escapeLeadingCharacter(char, nextChar, ignoreHead) {
	if (ignoreHead) return char;
	if (isAsciiNumber(char.codePointAt(0))) return `_${char}`;
	if (char === "-") {
		if (!nextChar) return `_${char}`;
		if (isAsciiNumber(nextChar.codePointAt(0))) return `_${char}`;
	}
	return char;
}
function escape(selectors, options) {
	...
	while (cursor < length) {
		...
		let replacement;
		if (codePoint > 127) replacement = `u_x${codePoint.toString(16)}_`;
		else if (usingDefaultMap) if (isHead) {
			const mapped = DEFAULT_ESCAPE_TABLE[codePoint];
			if (mapped !== void 0) replacement = mapped;
			else {
				...
				const escapedHead = escapeLeadingCharacter(char, next ? selectors.slice(nextIndex, nextIndex + next.size) : void 0, ignoreHead);
				if (escapedHead !== char) replacement = escapedHead;
			}
		} else {
			const mapped = DEFAULT_ESCAPE_TABLE[codePoint];
			if (mapped !== void 0) replacement = mapped;
		...
		if (replacement !== void 0) {
			if (!buffer) buffer = [];
			if (lastUnchangedIndex !== cursor) buffer.push(selectors.slice(lastUnchangedIndex, cursor));
			buffer.push(replacement);
			lastUnchangedIndex = cursor + size;
		}
		cursor += size;
	}
	if (!buffer) return selectors;
	if (lastUnchangedIndex < length) buffer.push(selectors.slice(lastUnchangedIndex));
	return buffer.join("");
}
//#endregion
//#region src/types.ts
...
function decodeUnicodeSequence(value, index) {
	if (!value.startsWith(UNICODE_PREFIX, index)) return;
	let cursor = index + 3;
	while (cursor < value.length) {
		const nextChar = value[cursor];
		if (!nextChar || !isHexDigit(nextChar)) break;
		cursor += 1;
	}
	if (cursor === index + 3) return;
	if (cursor >= value.length || value[cursor] !== UNICODE_SUFFIX) return;
	const hex = value.slice(index + 3, cursor);
	const codePoint = Number.parseInt(hex, HEX_RADIX);
	if (Number.isNaN(codePoint) || codePoint <= 127 || codePoint > 1114111) return;
	return {
...
//#endregion
//#region src/unescape.ts
const ESCAPE_PREFIX_CODE = "_".codePointAt(0);
const LOWER_U_CODE = "u".codePointAt(0);
const DEFAULT_UNESCAPE_TABLE = (() => {
	const inverse = {};
	for (const [char, token] of Object.entries(DEFAULT_ESCAPE_MAPPING)) inverse[token] = char;
	return inverse;
})();
function unescapeDefault(value, ignoreHead) {
	const length = value.length;
	const hasLeadingEscape = !ignoreHead && value[0] === "_";
	const hasUnicodeMarker = value.includes("u_x");
	if (!hasLeadingEscape && !hasUnicodeMarker) return value;
	let cursor = 0;
	let result = "";
	while (cursor < length) {
		const currentChar = value[cursor];
		if (value.codePointAt(cursor) === LOWER_U_CODE) {
	...
	if (ignoreHead || result.length === 0 || !result.startsWith("_")) return result;
	const nextChar = result[1];
	if (!nextChar) return result.slice(1);
	let shouldStrip = isAsciiNumber(nextChar.codePointAt(0));
	if (!shouldStrip && nextChar === "-") {
...
function unescape(value, options) {
	...
	const ignoreHead = options?.ignoreHead ?? false;
	if (!map) return unescapeDefault(value, ignoreHead);
	const usingDefaultMap = map === DEFAULT_ESCAPE_MAPPING;
	...
	while (cursor < length) {
		...
		if (replacement === void 0) {
			if (usingDefaultMap && currentCode === ESCAPE_PREFIX_CODE) {
				...
				if (bucket) {
					for (const token of bucket) if (value.startsWith(token, cursor)) {
		...
		if (replacement !== void 0) {
			if (!buffer) buffer = [];
			if (lastUnchangedIndex !== cursor) buffer.push(value.slice(lastUnchangedIndex, cursor));
			buffer.push(replacement);
	...
	const result = buffer ? (() => {
		if (lastUnchangedIndex < length) buffer.push(value.slice(lastUnchangedIndex));
		return buffer.join("");
	})() : value;
	if (ignoreHead || result.length === 0 || !result.startsWith("_")) return result;
	const nextChar = result[1];
