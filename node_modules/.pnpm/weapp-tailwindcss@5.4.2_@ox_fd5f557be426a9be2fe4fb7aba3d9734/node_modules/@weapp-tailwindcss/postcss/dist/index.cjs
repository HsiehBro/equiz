...
postcss_scss = require_rolldown_runtime.__toESM(postcss_scss, 1);
...
function rewriteApplyParams(params, marker) {
	const candidates = (0, _tailwindcss_mangle_engine.splitCandidateTokens)(params);
...
function injectUniAppXBaseDefaults(result, defaults) {
	...
	result.root.walkRules((rule) => {
		...
		for (const declaration of prependDecls.reverse()) rule.prepend(postcss.default.decl({
...
async function collectConfigContentFiles(root, base, options) {
	...
	for (const configPath of configPaths) {
		...
		}))?.config.content, node_path.default.dirname(configPath), { relativeBase: node_path.default.dirname(configPath) });
...
function isPureLocalCssImportWrapper(css) {
	try {
		return isPureLocalCssImportWrapperRoot(postcss.default.parse(css));
	} catch {
		return false;
	}
}
...
function splitLocalCssImports(source) {
	try {
		return splitLocalCssImportsRoot(postcss.default.parse(source));
	} catch {
		return;
	}
}
...
const buildFunctionNodes = (content) => (0, postcss_value_parser.default)(content).nodes;
function convertRgbFunction(fnNode) {
	...
	const normalizedColors = colorParts.map((part) => postcss_value_parser.default.stringify(part).trim()).filter(Boolean);
	if (normalizedColors.length !== 3) return false;
	const alphaText = postcss_value_parser.default.stringify(alphaNodes).trim();
...
function getPxTransformPlugin(options) {
	...
	if (options.px2rpx === true) return (0, postcss_pxtrans.default)(defaultPxTransformOptions);
	return (0, postcss_pxtrans.default)((0, _weapp_tailwindcss_shared.defuOverrideArray)(options.px2rpx, defaultPxTransformOptions));
}
//#endregion
...
function getUnitsToPxPlugin(options) {
	...
	if (userOptions?.disabled || userOptions?.transform === false) return (0, postcss_rule_unit_converter.default)({ disabled: true });
	const presetOptions = {};
	...
	converterOptions.rules = postcss_rule_unit_converter.presets.unitsToPx(presetOptions);
	return (0, postcss_rule_unit_converter.default)(converterOptions);
}
//#endregion
...
const beforeAfterParser = (0, postcss_selector_parser.default)((selectors) => {
...
function mklist(node) {
	return [
		node,
		postcss_selector_parser.default.combinator({ value: "+" }),
		node.clone()
	];
}
function composeIsPseudoAst(strs) {
	if (typeof strs === "string") return mklist(postcss_selector_parser.default.tag({ value: strs }));
	if (strs.length > 1) return mklist(postcss_selector_parser.default.pseudo({
		value: ":is",
		nodes: strs.map((str) => postcss_selector_parser.default.tag({ value: str }))
	}));
	return mklist(postcss_selector_parser.default.tag({ value: strs[0] }));
}
function getCombinatorSelectorAst(options) {
	let template = combinatorSelectorAstCache.get(options);
	if (!template) {
		template = mklist(postcss_selector_parser.default.tag({ value: "view" }));
...
function getFallbackRemove(_rule, options) {
	...
	if (!entry) {
		...
		const parser = (0, postcss_selector_parser.default)((selectors) => {
...
function createRuleTransformer(options) {
	...
	const parser = (0, postcss_selector_parser.default)(transform);
	return (rule) => {
...
exports.createPostcssStyleTargetProfile = createPostcssStyleTargetProfile;
...
exports.filterApplyOnlyGeneratedCssRoot = filterApplyOnlyGeneratedCssRoot;
...
exports.isLocalCssImportRequest = isLocalCssImportRequest;
exports.isMiniProgramLocalCssImportRequest = isMiniProgramLocalCssImportRequest;
...
exports.normalizeTailwindCssImportRequest = normalizeTailwindCssImportRequest;
...
exports.resolvePostcssStyleBranchProfile = resolvePostcssStyleBranchProfile;
exports.resolvePostcssStyleTarget = resolvePostcssStyleTarget;
exports.resolveSourceScanPath = resolveSourceScanPath;
...
exports.unwrapUnsupportedCascadeLayers = unwrapUnsupportedCascadeLayers;
