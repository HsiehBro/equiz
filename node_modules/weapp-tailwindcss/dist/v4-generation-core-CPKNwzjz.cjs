...
var AssetEmissionPlan = class {
	constructor() {
		require_object._defineProperty(this, "emissions", /* @__PURE__ */ new Map());
		require_object._defineProperty(this, "assetOperations", []);
	}
...
var CompilerShadowReportSession = class {
	constructor(maxReports = COMPILER_SHADOW_REPORT_CACHE_MAX) {
		...
		require_object._defineProperty(this, "reportsByScope", /* @__PURE__ */ new Map());
		require_object._defineProperty(this, "revision", 0);
		require_object._defineProperty(this, "dropped", 0);
		require_object._defineProperty(this, "completed", false);
		require_object._defineProperty(this, "disposed", false);
...
function collectApplyCandidates(root) {
	...
	root.walkAtRules("apply", (rule) => {
		for (const candidate of (0, _tailwindcss_mangle_engine.splitCandidateTokens)(rule.params)) candidates.add(candidate);
	});
	return candidates;
}
...
function resolveUniqueTailwindV4CssConfig(sourceOptions) {
	...
	const collect = (css, base) => {
		...
		const source = require_generator.resolveCssEntrySource(css, base, { removeConfig: false });
		if (source?.config) configs.add(node_path.default.resolve(source.config));
	};
	collect(sourceOptions.css, sourceOptions.base ?? sourceOptions.projectRoot);
	for (const source of sourceOptions.cssSources ?? []) {
		const base = source.base ?? (source.file ? node_path.default.dirname(node_path.default.resolve(source.file)) : void 0) ?? sourceOptions.base ?? sourceOptions.projectRoot;
...
async function resolveGeneratorSource(_majorVersion, runtimeState, rawSource, file, cssHandlerOptions, generatorOptions, selectionOptions) {
	...
	if (applyEntrySource && !cssHandlerOptions.isMainChunk && !require_generator.hasTailwindRootDirectives(rawSource, { importFallback: generatorOptions?.importFallback ?? false })) {
		...
		const css = createTailwindV4ApplyReferenceSource(require_generator.normalizeConfigDirective(require_generator.prependConfigDirective(applyEntrySource.css, config), void 0), normalizedSourceOptions ?? {});
...
async function appendLegacyCompatCss(css, rawSource, generatorTarget, styleHandler, cssHandlerOptions, generatorStyleOptions, options = {}) {
	...
	const transformedCompatCss = require_generator.removeTailwindV4GeneratedUserCssArtifacts(removeDuplicatedViteMarkers((0, _weapp_tailwindcss_postcss.removeUnsupportedMiniProgramAtRules)(compatCss), css));
	const cleanedCompatCss = options.preserveSelectorOverrides ? (0, _weapp_tailwindcss_postcss.filterExistingCssRules)(css, transformedCompatCss) : collectDedupedPostTransformCompatCss(transformedCompatCss, css);
...
async function finalizeFallbackGeneratorCss(context) {
	...
	if (generated.target === "weapp" && generatorRawSource.includes("weapp-tailwindcss generator-placeholder") && !require_generator.hasUserCssLayerBlocks(generatorRawSource)) css = createCssSourceOrderAppend(await require_generator.transformGeneratorUserCss(generatedUserCssRawSource, {
		generatorTarget: generated.target,
		generatorStyleOptions,
		cssUserHandlerOptions,
		styleHandler,
...
async function executeGeneratorPipelineWithOwner(context, compilationPool, generationSession) {
	...
	const preparedGenerationInputs = (await runWithConcurrency(generatorSourceRecords.map((record, index) => async () => {
		...
		return {
			generatorRuntime: require_generator.shouldUseMiniProgramCssBranch(generatorBranch) ? require_generator.filterUnsupportedMiniProgramTailwindV4Candidates(sourceRuntime) : sourceRuntime,
			generatorSource,
	...
	const outputContext = {
		...
		generated,
		generatedCss: filterGeneratedApplyOnlyCss(generated.target === "web" ? generated.css : require_generator.stripTailwindBanner(generated.css)),
		generatorStyleOptions,
		hasMatchedCssSourceFile,
		isolateCurrentCssCandidates,
		preflightMode,
...
async function generateCssByGenerator(options) {
	...
	const legacyCompatUserCssRawSource = generatorOptions.target === "weapp" && require_generator.hasUserCssLayerBlocks(generatedUserCssRawSource) ? require_generator.splitUserCssLayerBlocks(generatedUserCssRawSource).rest : generatedUserCssRawSource;
	const shouldPreserveLegacyCompatSelectorOverrides = legacyCompatUserCssRawSource !== generatedUserCssRawSource;
	const shouldDeduplicateUserLayerCss = generatorOptions.target === "weapp" && (require_generator.hasUserCssLayerBlocks(rawUserSource) || require_generator.hasUserCssLayerBlocks(generatorRawSource) || require_generator.hasUserCssLayerBlocks(generatedUserCssRawSource));
...
async function generateTailwindV4CssWithImplementation(options, implementation) {
	...
	const normalizedGeneratorOptions = require_generator.normalizeWeappTailwindcssGeneratorOptions(options.opts.generator, {
...
//#endregion
Object.defineProperty(exports, "AssetEmissionPlan", {
	enumerable: true,
	get: function() {
		return AssetEmissionPlan;
	}
});
...
Object.defineProperty(exports, "beginCompilerShadowRun", {
	enumerable: true,
	get: function() {
		return beginCompilerShadowRun;
	}
});
...
Object.defineProperty(exports, "collectEscapedRuntimeCandidates", {
	enumerable: true,
	get: function() {
		return collectEscapedRuntimeCandidates;
	}
});
...
Object.defineProperty(exports, "completeCompilerShadowReportRun", {
	enumerable: true,
	get: function() {
		return completeCompilerShadowReportRun;
	}
});
...
Object.defineProperty(exports, "createCompilationDependencyChanges", {
	enumerable: true,
	get: function() {
		return createCompilationDependencyChanges;
	}
});
...
Object.defineProperty(exports, "createSourceCandidateEligibilityMatcher", {
	enumerable: true,
	get: function() {
		return createSourceCandidateEligibilityMatcher;
	}
});
...
Object.defineProperty(exports, "disposeCompilerOwner", {
	enumerable: true,
	get: function() {
		return disposeCompilerOwner;
	}
});
...
Object.defineProperty(exports, "getCompilationScopeDependencyRevision", {
	enumerable: true,
	get: function() {
		return getCompilationScopeDependencyRevision;
	}
});
Object.defineProperty(exports, "getCompilerShadowRunSnapshot", {
	enumerable: true,
	get: function() {
		return getCompilerShadowRunSnapshot;
	}
});
...
Object.defineProperty(exports, "invalidateCompilationScope", {
	enumerable: true,
	get: function() {
		return invalidateCompilationScope;
	}
});
...
Object.defineProperty(exports, "recordCompilationDependencyChanges", {
	enumerable: true,
	get: function() {
		return recordCompilationDependencyChanges;
	}
});
...
Object.defineProperty(exports, "resolveCompilerMode", {
	enumerable: true,
	get: function() {
		return resolveCompilerMode;
	}
});
