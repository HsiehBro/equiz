...
function createTailwindGenerationArtifact(css, source, request, classSet, rawCandidates, dependencies) {
	...
	return {
		fragments: [{
			...
			kind: "tailwind",
			root: postcss.default.parse(css, { from: sourceId }),
			sourceId,
			order: 0
		}],
...
function resolveScanSources(options, source, compiledRoot, compiledSources) {
	...
	if (options?.scanSources === true) return require_style_candidates.createTailwindV4CompiledSourceEntries(compiledRoot, compiledSources, source.base);
	return [];
}
...
function stripCompiledSourceEntries(source) {
	...
	try {
		const root = postcss.default.parse(source.css);
		let changed = false;
...
async function collectRawCandidates(source, options, compiledRoot, compiledSources = []) {
	...
	for (const candidateSource of options?.sources ?? []) {
		const candidates = await require_style_candidates.extractRawCandidatesWithPositions(candidateSource.content, candidateSource.extension, extractOptions);
	...
	if (filesystemSources.length > 0) for (const candidate of await require_style_candidates.extractRawCandidates(filesystemSources, extractOptions)) rawCandidates.add(candidate);
	const inlineSources = require_style_candidates.extractTailwindV4InlineSourceCandidates(source.css);
...
var TailwindGenerationSessionImpl = class {
	...
	loadDesignSystem() {
		this.assertActive();
		this.designSystemPromise ??= require_style_candidates.loadTailwindV4DesignSystem(this.currentSource, { cache: false });
		return this.designSystemPromise;
	}
	async validateCandidates(candidates) {
		return require_style_candidates.resolveValidTailwindV4Candidates(await this.loadDesignSystem(), candidates);
	}
	invalidate(change) {
	...
	getRuntime(compileSourceEntries) {
		...
		const promise = Promise.all([require_style_candidates.compileTailwindV4Source(source), require_style_candidates.loadTailwindV4DesignSystem(source, { cache: false })]).then(([compiledSource, designSystem]) => ({
	...
	async generateInternal(request) {
		...
		const classSet = require_style_candidates.resolveValidTailwindV4Candidates(runtime.designSystem, rawCandidates, { ...options.bareArbitraryValues === void 0 ? {} : { bareArbitraryValues: options.bareArbitraryValues } });
		const inlineSources = require_style_candidates.extractTailwindV4InlineSourceCandidates(this.currentSource.css);
		for (const candidate of inlineSources.excluded) classSet.delete(candidate);
		const buildCandidates = new Set(require_style_candidates.canonicalizeBareArbitraryValueCandidates(classSet, options.bareArbitraryValues));
		...
		const css = require_style_candidates.replaceBareArbitraryValueSelectors(runtime.compiled.build([...buildCandidates]), classSet, options.bareArbitraryValues);
...
async function collectTailwindV4StyleCandidates(options) {
	return require_style_candidates.collectTailwindStyleCandidates({
		...options.bareArbitraryValues === void 0 ? {} : { bareArbitraryValues: options.bareArbitraryValues },
		...options.candidates === void 0 ? {} : { candidates: options.candidates },
...
Object.defineProperty(exports, "collectTailwindV4StyleCandidates", {
	enumerable: true,
	get: function() {
		return collectTailwindV4StyleCandidates;
	}
});
Object.defineProperty(exports, "createTailwindGenerationSession", {
	enumerable: true,
	get: function() {
		return createTailwindGenerationSession;
	}
});
...
Object.defineProperty(exports, "generateTailwindV4Style", {
	enumerable: true,
	get: function() {
		return generateTailwindV4Style;
	}
});
