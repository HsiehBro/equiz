...
function normalizeSourceFile$1(sourceFile) {
	return require_source_candidate_scan_signature.normalizeOutputPathKey(sourceFile.replace(/[?#].*$/, ""));
}
function normalizeOutputFile(outputFile) {
	return require_source_candidate_scan_signature.normalizeOutputPathKey(outputFile);
}
...
function createViteCssAssetIdentityResolver(options) {
	...
	const placeholderFile = require_source_candidate_scan_signature.normalizeOutputPathKey(node_path.default.resolve(options.generatorPlaceholderFile));
	return (asset, file) => {
		...
		const placeholderSourceFile = candidates.find((candidate) => require_source_candidate_scan_signature.normalizeOutputPathKey(node_path.default.resolve(candidate.replace(/[?#].*$/, ""))) === placeholderFile);
		let identity;
...
function createBundleModuleGraphOptions(outputDir, entries) {
	...
	for (const [id, entry] of entries) normalizedEntries.set(require_source_candidate_scan_signature.normalizeOutputPathKey(id), entry);
	const getEntry = (id) => entries.get(id) ?? normalizedEntries.get(require_source_candidate_scan_signature.normalizeOutputPathKey(id));
	return {
		resolve(specifier, importer) {
			return require_source_candidate_scan_signature.resolveOutputSpecifier(specifier, importer, outputDir, (candidate) => Boolean(getEntry(candidate)));
		},
		load(id) {
...
function applyLinkedResults(linked, entries, onLinkedUpdate, onApplied) {
	if (!linked) return;
	const normalizedEntries = /* @__PURE__ */ new Map();
	for (const [entryId, entry] of entries) normalizedEntries.set(require_source_candidate_scan_signature.normalizeOutputPathKey(entryId), entry);
	for (const [id, { code }] of Object.entries(linked)) {
		const entry = entries.get(id) ?? normalizedEntries.get(require_source_candidate_scan_signature.normalizeOutputPathKey(id));
		if (!entry) continue;
...
const removedBundleFilesStorage = new node_async_hooks.AsyncLocalStorage();
...
function classifyBundleEntry(file, opts) {
	return require_source_candidate_scan_signature.classifyRuntimeEntry(file, opts);
}
...
function resolveViteProcessedCssAssetSource(file, rawSource, resolveViteProcessedCssOutputFile) {
	const blocks = require_v4_generation_core.parseBundlerGeneratedCssMarkerBlocks(rawSource).filter((block) => block.bundler === "vite");
	if (blocks.length <= 1) return require_v4_generation_core.stripBundlerGeneratedCssMarkers(rawSource);
	...
	return matchedCss.length > 0 ? matchedCss.join("\n") : require_v4_generation_core.stripBundlerGeneratedCssMarkers(rawSource);
}
function collectMatchingGeneratedCssMarkerFiles(file, rawSource, resolveViteProcessedCssOutputFile) {
	return require_v4_generation_core.parseBundlerGeneratedCssMarkerBlocks(rawSource).filter((block) => block.bundler === "vite").filter((block) => isMatchingGeneratedCssMarkerFile(file, block.file, resolveViteProcessedCssOutputFile)).map((block) => block.file).filter((markerFile) => typeof markerFile === "string" && markerFile.length > 0);
}
...
function resolveSourceStyleSourceFromOutputFile(outputFile, snapshot, outputRoot, sourceRoot, getSourceStyleSource, getSourceStyleSources, configuredSourceEntries, debug) {
	...
	if (!sourceFile || !rawSource || !hasTailwindGenerationSourceForFile(sourceFile, rawSource)) {
		const cachedSources = [...getSourceStyleSources?.() ?? []].filter(([file, source]) => CSS_SOURCE_OUTPUT_EXT_RE.test(file) && hasTailwindGenerationSourceForFile(file, source)).map(([file, source]) => ({
			file,
			source,
			score: require_v4_generation_core.scoreTailwindV4CssSourceFileMatch(outputFile, file, {
				outputRoot,
...
function resolveSubpackageSourceRootFromModuleId(moduleId, subpackageRoot) {
	const file = slash(node_path.default.resolve(moduleId.replace(/[?#].*$/, "")));
...
function prepareImportedCssCoverage(importedCssSources) {
	const sources = importedCssSources.map((source) => require_v4_generation_core.stripBundlerGeneratedCssMarkers(source).trim()).filter(Boolean);
...
function resolveSingleCssImportOutputFile(targetFile, css) {
	...
	try {
		_weapp_tailwindcss_postcss.postcss.parse(css).walkAtRules("import", (atRule) => {
			...
			const request = require_generator.parseImportRequest(atRule.params);
...
function shouldPreserveFrameworkRootMiniProgramImportShell(options) {
	return !options.isWebGeneratorTarget && options.matchesCss && isRootMiniProgramStyleOutputFile$1(options.file) && (0, _weapp_tailwindcss_postcss.isPureLocalCssImportWrapper)(options.css) && shouldKeepRootMiniProgramStyleAsImportShell(options.shouldKeep());
}
function resolveFrameworkRootImportShellPlan(options) {
	...
	if (!(!options.isWebGeneratorTarget && isRootMiniProgramStyleOutputFile$1(options.rootImportShellOutputFile) && require_source_candidate_scan_signature.normalizeOutputPathKey(options.assetSourceFile) === require_source_candidate_scan_signature.normalizeOutputPathKey(options.file) && options.isMainChunk && shouldKeepRootMiniProgramStyleAsImportShell(options.shouldKeep()) && !shouldMoveRootMiniProgramStyleToImportShellOrigin(options.shouldMoveToOrigin()))) return {
		isCurrentImportShell,
		reusableTarget: void 0,
		targetToRemember: void 0
	};
	const targets = new Set([...options.configuredTargetFiles, ...options.processedTargetFiles].filter((file) => typeof file === "string" && isRootMiniProgramStyleOutputFile$1(file) && require_source_candidate_scan_signature.normalizeOutputPathKey(file) !== require_source_candidate_scan_signature.normalizeOutputPathKey(options.rootImportShellOutputFile)));
...
function restoreFrameworkRootMiniProgramImportShellAssets(bundle, options) {
	...
	for (const [sourceFile, targetFile] of options.targetByFile) {
		if (!options.matchesCss(sourceFile) || !options.matchesCss(targetFile) || !isRootMiniProgramStyleOutputFile$1(sourceFile) || !isRootMiniProgramStyleOutputFile$1(targetFile) || require_source_candidate_scan_signature.normalizeOutputPathKey(sourceFile) === require_source_candidate_scan_signature.normalizeOutputPathKey(targetFile)) continue;
		const output = Object.entries(bundle).find(([bundleFile, candidate]) => candidate.type === "asset" && require_source_candidate_scan_signature.normalizeOutputPathKey(candidate.fileName || bundleFile) === require_source_candidate_scan_signature.normalizeOutputPathKey(sourceFile))?.[1];
		if (output?.type !== "asset") continue;
...
function shouldUseCssAssetAsMainInjectionTarget(opts, file, records, options, bundle) {
	...
	const explicitStyleOutputTargets = records.filter((record) => record.injectIntoMain === true).map((record) => typeof record.outputFile === "string" ? require_source_candidate_scan_signature.normalizeOutputPathKey(record.outputFile) : void 0).filter((outputFile) => typeof outputFile === "string" && isRootStyleOutputFile(outputFile));
...
function collectViteProcessedCssAssetResults(bundle, options) {
	...
	for (const [bundleFile, output] of Object.entries(bundle)) {
		...
		if (require_source_candidate_scan_signature.normalizeOutputPathKey(recordOutputFile) !== require_source_candidate_scan_signature.normalizeOutputPathKey(file)) options.recordViteProcessedCssAssetResult?.(recordOutputFile, nextCss, {
			injectIntoMain: shouldReplayIntoMainCss || void 0,
			outputFile: recordOutputFile
		});
		for (const markerFile of collectMatchingGeneratedCssMarkerFiles(file, rawSource, options.resolveViteProcessedCssOutputFile)) {
			...
			if (require_source_candidate_scan_signature.normalizeOutputPathKey(recordOutputFile) !== require_source_candidate_scan_signature.normalizeOutputPathKey(markerFile) && require_source_candidate_scan_signature.normalizeOutputPathKey(recordOutputFile) !== require_source_candidate_scan_signature.normalizeOutputPathKey(file)) options.recordViteProcessedCssAssetResult?.(recordOutputFile, nextCss, {
				injectIntoMain: shouldReplayIntoMainCss || void 0,
				outputFile: recordOutputFile
			});
		}
...
function injectViteProcessedCssIntoMainCssAssets(bundle, options) {
	...
	for (const [bundleFile, bundleOutput] of Object.entries(bundle)) {
		...
		for (const record of viteCssResults) {
			...
			let css = measure("normalizeRecord", () => normalizeInjectableCssForTarget(require_v4_generation_core.stripBundlerGeneratedCssMarkers(record.css).trim(), file).trim());
			css = measure("importCoverage", () => removeCssCoveredByImportedViteResults(css, importedCssSources, importedCssCoverage).trim());
			if (css.length === 0) continue;
			const mergedLayerCss = measure("mergeLayers", () => require_v4_generation_core.mergeMarkedUserLayerComponentsCss(nextCss, css));
			if (mergedLayerCss.merged) {
				nextCss = mergedLayerCss.css;
				css = require_v4_generation_core.extractMarkedUserLayerComponentsCss(css).rest.trim();
				if (css.length === 0) continue;
			}
...
function isStringRuleMatched(rule, id, rootDir) {
	...
	const relative = slash(node_path.default.relative(rootDir, id));
...
function processViteCssCacheTask(options) {
	return require_hmr_timing.processCachedTask({
...
function findRememberedCssSources(sources, outputFile, file, originalSource, outputRoot, sourceRoot) {
	...
	const outputKeys = new Set([outputFile, file].map(require_source_candidate_scan_signature.normalizeOutputPathKey));
	return rememberedSources.filter((remembered) => [...relationOwner.getOwnedOutputs(remembered.sourceFile)].some((ownedOutput) => outputKeys.has(require_source_candidate_scan_signature.normalizeOutputPathKey(ownedOutput))));
}
...
function resolveViteCssCompositionPlan(options) {
	...
	const rootImportShellTarget = !options.isWebGeneratorTarget && options.isRootStyleOutputFile(options.rootImportShellOutputFile) && options.isRootStyleOutputFile(outputFile) && require_source_candidate_scan_signature.normalizeOutputPathKey(options.rootImportShellOutputFile) !== require_source_candidate_scan_signature.normalizeOutputPathKey(outputFile) && options.shouldKeepRootImportShell && !options.shouldMoveRootImportShellToOrigin ? outputFile : void 0;
	const useRememberedCssSource = !options.shouldKeepImportedCssShell && rememberedSource != null && (require_source_candidate_scan_signature.normalizeOutputPathKey(rememberedSource.sourceFile) !== require_source_candidate_scan_signature.normalizeOutputPathKey(options.file) || !hasTailwindGenerationSource(options.rawSource) && hasTailwindGenerationSource(rememberedSource.rawSource));
	...
	const hasCurrentTailwindGenerationDirective = require_generator.hasTailwindSourceDirectives(options.rawSource, { importFallback: true }) || require_generator.hasTailwindRootDirectives(options.rawSource, { importFallback: true }) || require_generator.hasTailwindApplyDirective(options.rawSource);
	const preserveImportedCssShell = options.shouldKeepImportedCssShell && !hasCurrentTailwindGenerationDirective;
	const hasRememberedApplySource = vitePipelineCssAsset && rememberedSource != null && require_generator.hasTailwindApplyDirective(generatorRawSource);
	...
	const hasRememberedApplyDirective = rememberedSource != null && require_generator.hasTailwindApplyDirective(rememberedSource.rawSource);
	const hasRememberedTailwindGenerationSource = rememberedSource != null && hasTailwindGenerationSource(rememberedSource.rawSource);
	const hasSameOutputRememberedTailwindGenerationSource = hasRememberedTailwindGenerationSource && rememberedSource != null && require_source_candidate_scan_signature.normalizeOutputPathKey(rememberedSource.outputFile) === require_source_candidate_scan_signature.normalizeOutputPathKey(outputFile);
	...
	const usesConfiguredTailwindV4FallbackSource = rememberedSource != null && require_source_candidate_scan_signature.normalizeOutputPathKey(rememberedSource.outputFile) === require_source_candidate_scan_signature.normalizeOutputPathKey(outputFile) && require_source_candidate_scan_signature.normalizeOutputPathKey(rememberedSource.sourceFile.replace(/[?#].*$/, "")) !== require_source_candidate_scan_signature.normalizeOutputPathKey(options.file);
...
function collectConfiguredTailwindV4CssSources(opts) {
	...
	return require_generator.filterTailwindV4CssSourceRoots([...opts.tailwindcss?.v4?.cssSources ?? [], ...runtimeCssSources]) ?? [];
}
...
function collectConfiguredTailwindV4CssSourceEntries(opts, fallbackBase) {
	...
	for (const cssSource of collectConfiguredTailwindV4CssSources(opts)) {
		...
		const base = require_generator.resolveTailwindV4CssSourceBase(cssSource, fallbackBase);
...
function collectCssConfigBaseCandidates(source, file, outputRoot, opts) {
	...
	for (const cssSource of collectConfiguredTailwindV4CssSources(opts)) {
		...
		addCandidate(require_generator.resolveTailwindV4CssSourceBase(cssSource, sourceBaseFallback));
	}
	return candidates;
}
function normalizeRelativeCssConfigDirectives(source, file, outputRoot, opts) {
	...
	return source.replace(/@config\s+(["'])(.+?)\1\s*;?/g, (full, quote, request) => {
		...
		for (const base of baseCandidates) {
			...
			if ((0, node_fs.existsSync)(configFile)) return `@config ${quote}${slash(configFile)}${quote};`;
		}
		return full;
	});
}
...
function createCssHandlerOptionsCache(options) {
	...
	const getCssHandlerOptions = (file) => {
		...
		const sourceFile = require_v4_generation_core.stripRequestQuery(file);
...
function shouldSkipRawSourceStyleAsset(outputFile, file, rawSource, assetSourceFile = file, cssMatcher) {
	...
	const isKnownStyleSource = require_v4_generation_core.isSourceStyleRequest(assetSourceFile) || cssMatcher?.(assetSourceFile) === true;
...
function resolveCssAssetOutputPlan(options) {
	...
	const resolvedFromConfiguredOriginalCssEntry = configuredOriginalOutputFile != null && require_source_candidate_scan_signature.normalizeOutputPathKey(configuredOriginalOutputFile) !== require_source_candidate_scan_signature.normalizeOutputPathKey(outputFile);
...
function createCssTransformShareScopeKey(opts, file, rawSource) {
	...
	if (require_generator.hasTailwindGeneratedCssMarkers(rawSource) || require_generator.hasTailwindSourceDirectives(rawSource, { importFallback: generatorOptions.importFallback })) return `source:${require_source_candidate_scan_signature.normalizeOutputPathKey(file)}`;
...
async function resolveRememberedCssSourcePlan(options) {
	...
	sources = sources.filter((remembered) => {
		...
		return !matchedOutputFile || require_source_candidate_scan_signature.normalizeOutputPathKey(matchedOutputFile) === require_source_candidate_scan_signature.normalizeOutputPathKey(options.outputFile);
	});
	let hasUsableTailwindSource = sources.some((remembered) => hasTailwindGenerationSource(remembered.rawSource) && require_source_candidate_scan_signature.normalizeOutputPathKey(remembered.sourceFile.replace(/[?#].*$/, "")) !== require_source_candidate_scan_signature.normalizeOutputPathKey(options.file));
	...
	if (!hasUsableTailwindSource && options.temporaryOutput && hasTailwindGenerationSource(options.rawSource)) {
		...
		if (temporarySource) {
			...
			usedConfiguredSourceFiles.push(require_source_candidate_scan_signature.normalizeOutputPathKey(temporarySource.sourceFile));
			sources = [{
				outputFile,
...
function selectTailwindV4GenerationCssSourceForOutput(outputFile, entries, rawSource, options = {}) {
	...
	const selectByOutputPath = (candidates, shouldUseScore = (score) => score > 0) => {
		...
		const scoredSources = candidates.map((entry) => ({
			entry,
			score: require_v4_generation_core.scoreTailwindV4CssSourceFileMatch(outputFile, entry.file, {
				cwd: options.cwd,
...
function createResolvedPlan(plan, resolution, source, options = {}) {
	return {
		...
		sources: [source],
		usedConfiguredSourceFiles: options.usedConfiguredSourceFile ? [require_source_candidate_scan_signature.normalizeOutputPathKey(options.usedConfiguredSourceFile)] : []
	};
}
...
async function resolveViteCssSourcePlan(options) {
	...
	if (options.isCurrentRootMiniProgramStyleOutput && (hasTailwindGenerationSource(options.rawSource) || configuredEntries.length > 0)) {
		...
		if (source) {
			...
			if (!options.isConfiguredSourceUsed(source.file) && !options.isConfiguredSourceProcessed(source.file) || require_source_candidate_scan_signature.normalizeOutputPathKey(outputFile) === require_source_candidate_scan_signature.normalizeOutputPathKey(options.outputFile)) {
	...
	if (originalSource) {
		const outputFile = require_source_candidate_scan_signature.normalizeOutputPathKey(originalSource.outputFile) === require_source_candidate_scan_signature.normalizeOutputPathKey(options.outputFile) ? options.outputFile : options.resolveMatchedOutputFile(originalSource.sourceFile) ?? options.outputFile;
...
function resolveViteCssTransformDecisionPlan(options) {
	...
	const shouldPreserveCollectedViteCssAsset = !shouldRegenerateCollectedViteCss && (!options.generatorCandidateSignatureInitialized || !options.generatorCandidatesChanged) && (options.isCollectedBundlerGeneratedCssFile || require_v4_generation_core.hasBundlerGeneratedCssMarker(options.rawSource));
	const strippedViteProcessedCss = require_v4_generation_core.stripBundlerGeneratedCssMarkers(options.rawSource);
	const shouldPreserveStaleGeneratedCssAsset = options.hasStaleViteProcessedCssSource && shouldPreserveCollectedViteCssAsset && strippedViteProcessedCss.trim().length > 0 && options.cssAssetIdentityKind !== "generator-placeholder" && !hasTailwindGenerationSource(strippedViteProcessedCss) && !require_generator.hasTailwindApplyDirective(strippedViteProcessedCss);
...
function applyViteCssTransformTaskResult(options) {
	...
	if (options.debugCssDiff && result.diffSource != null) options.debug("css diff %s: %s", options.generatorSourceFile, require_source_candidate_scan_signature.summarizeStringDiff(result.diffSource, result.css));
...
function resolveGeneratorUserRawSource(options) {
	const bundleUserRawSource = options.vitePipelineCssAsset && !require_v4_generation_core.hasBundlerGeneratedCssMarker(options.rawSource) && normalizeCssSourceForCompare(options.rawSource) !== normalizeCssSourceForCompare(options.generatorRawSource) ? options.normalizeMiniProgramGeneratorRawSource(options.normalizeGeneratorUserRawSource(options.rawSource, options.generatorSourceFile, options.assetSourceFile), options.outputFile) : void 0;
...
async function executeViteCssTransformTask(options) {
	...
	if (hasTailwindGenerationSource(options.generatorRawSource) || !options.isWebGeneratorTarget && require_v4_generation_core.hasBundlerGeneratedCssMarker(options.rawSource)) {
		const generatorUserRawSource = resolveGeneratorUserRawSource(options);
		const generated = await (options.generateCss ?? require_v4_generation_core.generateTailwindV4Css)({
	...
	if ((0, _weapp_tailwindcss_postcss.isPureLocalCssImportWrapper)(options.generatorRawSource)) return {
...
async function handleUniAppXPostCssTasks(options) {
	...
	if (isHarmonyAppStyleTarget && applyUtilities.size > 0 && applyStyleSources.length > 0) {
		...
		if (generated?.css) viteProcessedCssSources.push(require_v4_generation_core.annotateCssSourceTrace(generated.css, {
			opts,
			tokenSources: getSourceCandidateSourcesForEntries ? require_v4_generation_core.createCssTokenSourceMap(getSourceCandidateSourcesForEntries(void 0), opts) : void 0
		}));
	}
...
function finalizeWebviewCssCompat(bundle, options) {
	const generatorOptions = require_generator.normalizeWeappTailwindcssGeneratorOptions(options.opts.generator);
...
function processHtmlBundleEntry(options) {
	...
	const htmlProcessHash = `${cache.computeHash(currentBundleSource)}:${cache.computeHash(require_v4_generation_core.createRuntimeAffectingSourceSignature(currentBundleSource, "html"))}:${templateRuntimeSignature}`;
...
function createUniAppXNativeBuildTargetResolver(getResolvedConfig) {
	...
	return (id) => {
		...
		const detected = require_generator.resolveUniUtsPlatform().isApp || isUniAppXNativeAppOutDir(config?.build?.outDir) || isUniAppXNativeAppOutDir(config?.root) || isUniAppXNativeAppOutDir(id);
...
async function processRememberedCssReplay(options) {
	...
	for (const [rememberedOutputFile, rememberedGroup] of rememberedReplayGroups) {
		...
		}) || !isRootMiniProgramStyleOutputFile$1(outputFile) && !isWebGeneratorTarget && (0, _weapp_tailwindcss_postcss.isPureLocalCssImportWrapper)(rawSource);
		...
		if (shouldPreserveLocalImportWrapper) {
			cssTaskFactories.push(() => timeTask("css.replay", async () => {
				const start = performance.now();
				const css = annotateCss(require_v4_generation_core.normalizeMiniProgramImportShell(rawSource));
...
function createConfiguredCssSourceRegistry(options) {
	const runtimeTailwindcssOptions = require_generator.resolveTailwindcssOptions(options.opts.tailwindcssRuntimeOptions);
	const tailwindRuntimeOptions = require_generator.resolveTailwindcssOptions(options.runtimeState.tailwindRuntime.options);
	...
	const normalizeConfiguredCssSourceCacheKey = (file) => normalizeSystemAliasPathKey(require_source_candidate_scan_signature.normalizeOutputPathKey(node_path.default.resolve(file.replace(/[?#].*$/, ""))));
	const getSourceEntries = () => {
		const collectedEntries = collectConfiguredTailwindV4CssSourceEntries({
			...
			tailwindcssRuntimeOptions: {
				...
				tailwindcss: {
					...require_generator.resolveTailwindcssOptions(options.opts.tailwindcssRuntimeOptions) ?? {},
					...require_generator.resolveTailwindcssOptions(options.runtimeState.tailwindRuntime.options) ?? {}
				}
			}
		}, options.opts.tailwindcssBasedir ?? options.rootDir);
		const cachedEntries = explicitCssEntryFiles.flatMap((file) => {
			const resolvedFile = node_path.default.isAbsolute(file) ? node_path.default.resolve(file) : node_path.default.resolve(options.opts.tailwindcssBasedir ?? options.rootDir, file);
...
function createConfiguredCssRootResolvers(options) {
	const normalizeEntryFile = (file) => {
		const cleanFile = file.replace(/[?#].*$/, "");
		return normalizeSystemAliasPathKey(require_source_candidate_scan_signature.normalizeOutputPathKey(node_path.default.isAbsolute(cleanFile) ? node_path.default.resolve(cleanFile) : node_path.default.resolve(options.opts.tailwindcssBasedir ?? options.rootDir, cleanFile)));
	};
	...
	const selectSourceEntry = (outputFile, entries, originalFileNames) => {
		...
		const matchedOutputEntries = generationEntries.filter((entry) => require_source_candidate_scan_signature.normalizeOutputPathKey(resolveEntryOutputFile(entry.file).replace(/[?#].*$/, "")) === require_source_candidate_scan_signature.normalizeOutputPathKey(outputFile.replace(/[?#].*$/, "")));
		...
		const rootOutputEntries = generationEntries.filter((entry) => !require_source_candidate_scan_signature.normalizeOutputPathKey(resolveEntryOutputFile(entry.file).replace(/[?#].*$/, "")).includes("/"));
	...
	const shouldKeepRootImportShell = (outputFile, css) => {
		const normalizedOutputFile = require_source_candidate_scan_signature.normalizeOutputPathKey(outputFile.replace(/[?#].*$/, ""));
	...
	return {
		...
		hasExplicitConfiguredRootCssEntryForOutput: (outputFile) => {
			...
			return options.entries.some((entry) => {
				...
				return !require_source_candidate_scan_signature.normalizeOutputPathKey(entryOutputFile.replace(/[?#].*$/, "")).includes("/") || options.currentSubpackageRoots != null && !isSubpackageOutputFile(entryOutputFile, options.currentSubpackageRoots);
			});
		},
		isMiniProgramStyleOutputFile,
...
function shouldQueueTemporarySource(entry, options) {
	return require_source_candidate_scan_signature.normalizeOutputPathKey(entry.outputFile).includes("/") || options.explicitSourceFileKeys.has(options.normalizeConfiguredSourceFile(entry.file)) && options.shouldSelectConfiguredRootOutput(entry.outputFile);
}
function collectTemporaryCssSourceEntries(options) {
	...
	const configuredEntries = options.configuredEntries.filter((entry) => require_source_candidate_scan_signature.normalizeOutputPathKey(entry.outputFile).includes("/") || shouldQueueTemporarySource(entry, options));
...
async function validateRuntimeCandidates(options) {
	...
	if (require_generator.hasTailwindApplyDirective(mainCssRawSource)) return;
	...
	const validatedSourceRuntime = await require_v4_generation_core.validateCandidatesByGenerator({
	...
	return async function generateBundle(_opt, bundle) {
		...
		const transformWebTargetCss = (css) => {
			return context.cssPipelineStrategy?.transformGeneratedCss?.(css, {
				...cssPipelineContext,
				defaultWebCssCompat: (value) => (0, _weapp_tailwindcss_postcss.transformWebCssCompat)(value, resolveViteWebCssCompatOptions(cssPipelineContext)),
				removeScopedPreflight: (value) => value,
				shouldApplyWebCssCompat
			}) ?? (shouldApplyWebCssCompat ? (0, _weapp_tailwindcss_postcss.transformWebCssCompat)(css, resolveViteWebCssCompatOptions(cssPipelineContext)) : css);
		};
		...
		const normalizeMiniProgramGeneratorRawSource = (source, outputFile) => {
			return isWebGeneratorTarget ? source : require_v4_generation_core.normalizeMiniProgramGeneratorCssSource(source, outputFile);
		};
		await runtimeState.readyPromise;
		debug("start");
		onStart();
		const collectedBundlerGeneratedCssFiles = new Set(Object.entries(bundle).filter(([, output]) => output.type === "asset" && require_v4_generation_core.hasBundlerGeneratedCssMarker(output.source)).map(([file]) => file));
		...
		for (const entry of snapshot.entries) {
			...
			if (processStyles && type === "css" && originalSource.type === "asset") {
				await processViteCssBundleEntry({
					...
					addWatchFile,
					annotateCssSourceTrace: require_v4_generation_core.annotateCssSourceTrace,
					applyCssResultToBundle,
					applyViteCssCacheResult,
					applyViteCssTransformTaskResult,
					bundle,
					...
					context,
					createCandidateSignature: require_source_candidate_scan_signature.createCandidateSignature,
					createCssSourceTraceCacheSignature: require_v4_generation_core.createCssSourceTraceCacheSignature,
					createCssTokenSourceMap: require_v4_generation_core.createCssTokenSourceMap,
					createCssTransformShareScopeKey,
					createInitialCssPipelineContext,
					createRuntimeAffectingSourceSignature: require_v4_generation_core.createRuntimeAffectingSourceSignature,
					createScopedGeneratorCandidateSignature: require_source_candidate_scan_signature.createScopedGeneratorCandidateSignature,
					createScopedGeneratorRuntime: createScopedGeneratorRuntime$1,
					createScopedGeneratorSourceTraceMap: require_source_candidate_scan_signature.createScopedGeneratorSourceTraceMap,
					createScopedSourceCandidateGetter,
					createScopedSourceCandidateSourceGetter,
					cssTaskFactories,
					currentSubpackageRoots,
					...
					normalizeGeneratorUserRawSource,
					normalizeMiniProgramGeneratorRawSource,
					normalizeOutputPathKey: require_source_candidate_scan_signature.normalizeOutputPathKey,
...
function createGenerateBundleHook(context) {
	...
	return async function generateBundle(options, bundle) {
		...
		require_v4_generation_core.beginCompilerShadowRun(context.runtimeState);
...
function shouldGenerateCssByGenerator(opts, majorVersion, file, rawSource, processed) {
	...
	if (require_generator.hasLocalCssImport(rawSource)) return false;
	if (require_generator.hasTailwindGeneratedCssMarkers(rawSource)) return true;
	if (require_generator.hasTailwindRootDirectives(rawSource, { importFallback: generatorOptions.importFallback })) return true;
	return processed && require_generator.hasTailwindApplyDirective(rawSource) && shouldFinalizeProcessedCssAsset(opts, file);
}
...
function finalizeWebCss(css, context, cssPipelineStrategy) {
	...
	const defaultWebCssCompat = (value) => (0, _weapp_tailwindcss_postcss.transformWebCssCompat)(value, resolveViteWebCssCompatOptions(context));
...
function createViteCssFinalizerOutputPlugin(context) {
	return {
		...
		async closeBundle() {
			...
			await require_v4_generation_core.disposeCompilerOwner(context.runtimeState);
		},
		async closeWatcher() {
			await require_v4_generation_core.disposeCompilerOwner(context.runtimeState);
		},
		generateBundle: {
			...
			async handler(_options, bundle) {
				...
				const generatorRuntime = require_generator.shouldUseMiniProgramCssBranch(generatorBranch) ? require_generator.filterUnsupportedMiniProgramTailwindV4Candidates(collectedGeneratorCandidates) : collectedGeneratorCandidates;
				const emissionPlan = new require_v4_generation_core.AssetEmissionPlan();
				const writeTargets = /* @__PURE__ */ new Map();
				...
				await Promise.all(entries.map(async ([bundleFile, output]) => {
					...
					if (isViteProcessedCssAsset?.(output, file)) {
						const cleanRawSource = require_v4_generation_core.stripBundlerGeneratedCssMarkers(rawSource);
					...
					const generatorRawSource = generatorBranch.isWeb ? resolvedGeneratorRawSource : require_v4_generation_core.normalizeMiniProgramGeneratorCssSource(resolvedGeneratorRawSource, file);
					...
					const generated = shouldGenerateCssByGenerator(opts, runtimeState.tailwindRuntime.majorVersion, file, generatorTransformRawSource, processed) ? await require_v4_generation_core.generateTailwindV4Css({
						opts,
						runtimeState,
					...
					if (!generated && !generatorBranch.isWeb && (0, _weapp_tailwindcss_postcss.isPureLocalCssImportWrapper)(generatorTransformRawSource)) {
...
function normalizeCssSourceIdentity(sourceFile) {
	...
	return require_source_candidate_scan_signature.normalizeOutputPathKey(require_v4_generation_core.stripRequestQuery(cleanSourceFile));
}
...
function createViteCssMemory(options) {
	...
	const resolveCachedStyleSource = (sourceFile) => {
		const file = (0, _weapp_tailwindcss_shared.cleanUrl)(require_v4_generation_core.stripRequestQuery(sourceFile));
		if (SFC_COMPONENT_FILE_RE.test(file)) return getKnownSfcSource(file);
		if (require_v4_generation_core.isSourceStyleRequest(file)) return options.getSourceCandidateSource(file);
	};
	...
	const refreshRememberedCssSourceByCurrentFile = async (sourceFile) => {
		...
		const matchedRememberedSources = [...rememberedCssSources.values()].filter((remembered) => require_source_candidate_scan_signature.normalizeOutputPathKey(require_v4_generation_core.stripRequestQuery((0, _weapp_tailwindcss_shared.cleanUrl)(remembered.sourceFile))) === normalizedSourceFile);
		...
		if (require_v4_generation_core.isSourceStyleRequest(file)) refreshRememberedCssSourceBySourceFile(file, source);
	};
...
function isCssLikeImporter(importer) {
	...
	return require_v4_generation_core.isSourceStyleRequest(importer) || isCSSRequest(normalized) || normalized.endsWith("/*");
}
...
function createRewriteCssImportsPlugins(options) {
	...
	return [{
		...
		async transform(code, id) {
			...
			const rewritten = require_css_imports.rewriteTailwindcssImportsInCode(normalizedCode, weappTailwindcssDirPosix, {
				join: joinPosixPath,
				appType: resolveAppType(),
				rootImport
			});
...
function hasVitePipelineTailwindGenerationDirective(code) {
	return require_generator.hasTailwindRootDirectives(code) || require_generator.hasTailwindApplyDirective(code);
}
//#endregion
...
function createViteRuntimeClassSet(options) {
	...
	const transformRuntimeClassSetManager = require_hmr_timing.createRuntimeClassSetManager({
		bareArbitraryValues: opts.arbitraryValues?.bareArbitraryValues,
		escapeMap: opts.escapeMap
	});
	let runtimeSet;
	...
	function resolveRuntimeRefreshOptions() {
		const configPath = require_generator.resolveTailwindcssOptions(runtimeState.tailwindRuntime.options)?.config;
...
function isViteServeStyleRequest(id, command) {
	return command === "serve" && require_v4_generation_core.isSourceStyleRequest(id) && !SPECIAL_QUERY_RE$1.test(id) && !COMMON_JS_PROXY_RE.test(id);
}
function isViteServeCssRootRequest(id, command) {
	return command === "serve" && require_v4_generation_core.isSourceStyleRequest(id) && !SPECIAL_QUERY_RE$1.test(id) && !COMMON_JS_PROXY_RE.test(id);
}
function isViteBuildStyleRequest(id, command) {
	return command === "build" && require_v4_generation_core.isSourceStyleRequest(id) && !SPECIAL_QUERY_RE$1.test(id) && !COMMON_JS_PROXY_RE.test(id);
}
...
function resolveViteServeRootMiniProgramImportShell(options) {
	...
	}) ? require_v4_generation_core.normalizeMiniProgramImportShell(css) : void 0;
}
//#endregion
...
function resolveImplicitTailwindcssBasedirFromViteRoot(root) {
	...
	const packageRoot = require_generator.findNearestPackageRoot(resolvedRoot);
	...
	const plugin = {
		...
		config(config) {
			...
			return (0, _weapp_tailwindcss_postcss.resolvePostcssConfig)(root).then((postcssConfig) => {
				...
				const removed = (0, _weapp_tailwindcss_postcss.removeTailwindPostcssPlugins)(plugins);
			...
			await options.hmrTimingRecorder.measure("configResolved", async () => {
				...
				if (options.shouldInferAppType && resolvedRoot) {
					...
					if (nextAppType && options.opts.appType !== nextAppType) {
						...
						_weapp_tailwindcss_logger.logger.debug("根据 Vite 项目根目录自动推断 appType -> %s", nextAppType);
				...
				if (options.frameworkName === "generic" && !options.hasExplicitGeneratorTarget && options.opts.generator !== false && options.opts.appType === void 0 && options.opts.platform === void 0 && options.opts.cssOptions?.platform === void 0 && !options.resolveViteStylePlatform()) {
					...
					_weapp_tailwindcss_logger.logger.debug("Generic Vite 项目未指定目标，自动使用 generator.target -> web");
				...
				require_v4_generation_core.captureFrameworkPostcssOptions(options.opts, config.css?.postcss && typeof config.css.postcss === "object" ? config.css.postcss : void 0);
			}, { emit: false });
		},
		generateBundle: {
			order: "post",
...
function createFrameworkProcessedCssRegistry() {
	...
	const markSource = (file) => {
		sourceFiles.add(node_path.default.resolve((0, _weapp_tailwindcss_shared.cleanUrl)(file)));
	};
	...
	const matchesIdentity = (candidate) => {
		return sourceFiles.has(node_path.default.resolve((0, _weapp_tailwindcss_shared.cleanUrl)(candidate)));
	};
	...
	return {
		...
		transform: {
			...
			async handler(code, id) {
				...
				if (!options.shouldOwnTailwindGeneration || shouldSkipSourceCandidateState() || options.collectSourceCandidates === false || !require_v4_generation_core.isSourceCandidateRequest(id) || !shouldCollectTransformedSourceCandidates(id)) return shouldReturnTransformedCode ? {
...
function createFrameworkSourceScanSession(options) {
	...
	const collectRoots = (root, entries) => {
		...
		const seenRoots = /* @__PURE__ */ new Set([node_path.default.resolve(root)]);
		const basedir = options.opts.tailwindcssBasedir ? node_path.default.resolve(options.opts.tailwindcssBasedir) : void 0;
		...
		for (const cssEntry of options.opts.tailwindcss?.v4?.cssEntries ?? []) {
			if (!require_generator.isTailwindV4CssEntry(cssEntry)) continue;
...
function createFrameworkTailwindRootCss(options) {
	...
	const refreshSource = async (id, code) => {
		const file = (0, _weapp_tailwindcss_shared.cleanUrl)(id);
		if (!moduleIds.has(id) && !moduleIds.has(file) && !require_generator.hasTailwindRootDirectives(code, { importFallback: options.getImportFallback() })) return;
...
const generatorPlaceholderCssFile = node_path.default.join(weappTailwindcssPackageDir, "generator-placeholder.css");
...
function createViteFrameworkPlugins$1(options = {}, frameworkBranch) {
	...
	const syncCssEntriesFromAnchor = (anchor) => {
		...
		const normalizedCssEntries = require_generator.normalizeCssEntries(rawCssEntries, anchor ?? node_process.default.cwd());
	...
	const resolveCurrentGeneratorOptions = () => require_generator.normalizeWeappTailwindcssGeneratorOptions(opts.generator, {
		appType: opts.appType,
	...
	const resolveCurrentGeneratorBranch = () => require_generator.resolveGeneratorRuntimeBranch(resolveCurrentGeneratorOptions(), {
		appType: opts.appType,
		platform: resolveGeneratorPlatform(),
		tailwindcssMajorVersion,
		uniAppX
	...
	const hasInitialTailwindCssRoots = require_generator.hasConfiguredTailwindV4CssRoots({
		...rawOptions,
	...
	const registerAutoCssSource = async (id, css, options2 = {}) => {
		...
		const file = (0, _weapp_tailwindcss_shared.cleanUrl)(id);
		if (!node_path.default.isAbsolute(file)) return;
		if (!require_generator.isTailwindV4CssEntry(file)) return;
		...
		const sourceCss = require_generator.normalizeTailwindSourceForGenerator(require_generator.normalizeTailwindConfigDirectives(css, sourceBase), { importFallback: true });
	...
	const discoverAndRegisterAutoCssSources = async () => {
		...
		for (const cssEntry of cssEntries) {
			...
			const sourceBase = node_path.default.dirname(sourceFile);
			const sourceCss = require_generator.normalizeTailwindSourceForGenerator(require_generator.normalizeTailwindConfigDirectives(await (0, node_fs_promises.readFile)(sourceFile, "utf8"), sourceBase), { importFallback: true });
	...
	const originalCssLayerSourceByFile = new lru_cache.LRUCache({ max: 128 });
	const rememberOriginalCssLayerSource = (id, code) => {
		const file = (0, _weapp_tailwindcss_shared.cleanUrl)(id);
		if (!isCSSRequest(file)) return;
		if (!require_generator.hasUserCssLayerBlocks(code)) {
			originalCssLayerSourceByFile.delete(file);
			return;
		}
		originalCssLayerSourceByFile.set(file, require_generator.splitUserCssLayerBlocks(code).layer);
	};
	...
	const readCssAssetSource = (asset) => {
		return typeof asset.source === "string" ? asset.source : asset.source instanceof Uint8Array ? node_buffer.Buffer.from(asset.source).toString() : String(asset.source ?? "");
	};
	const markCssAssetProcessed = (asset, file) => {
		processedCssAssets.add(asset);
		if (file) processedCssAssetSourceByFile.set(require_source_candidate_scan_signature.normalizeOutputPathKey(file), readCssAssetSource(asset));
	};
	const isCssAssetProcessed = (asset, file) => {
		if (processedCssAssets.has(asset)) return true;
		if (!file) return false;
		const source = readCssAssetSource(asset);
		if (processedCssAssetSourceByFile.get(require_source_candidate_scan_signature.normalizeOutputPathKey(file)) === source) return true;
	...
	const sourceScanSession = createFrameworkSourceScanSession({
		...
		hmrCandidateState,
		isCandidateRequest: require_v4_generation_core.isSourceCandidateRequest,
		isWatchLikeBuild,
		opts,
		runtimeState,
	...
	const generateTailwindCssForVitePipelineNow = async (id, code, hookContext) => {
		...
		const generatorTransformCode = currentGeneratorBranch.isWeb ? generatorCode : require_v4_generation_core.normalizeMiniProgramGeneratorCssSource(generatorCode, outputFile);
...
${previousTracedCss}`;
		...
		const transientCssSource = transientAutoCssSources.get(file) ?? (require_generator.hasTailwindRootDirectives(generatorTransformCode, { importFallback: currentGeneratorOptions.importFallback }) || require_generator.hasTailwindSourceDirectives(generatorTransformCode, { importFallback: currentGeneratorOptions.importFallback }) || require_generator.hasTailwindApplyDirective(generatorTransformCode) ? {
		...
		const previousGeneratorCss = previousCss && !currentGeneratorBranch.isWeb ? require_v4_generation_core.normalizeMiniProgramGeneratorCssSource(previousCss, outputFile) : previousCss;
		const hmrDebugState = hmrCandidateState.snapshotDebugState();
		const generated = await hmrTimingRecorder.measure(`generateCss.${resolvedConfig?.command ?? "unknown"}`, () => require_v4_generation_core.generateTailwindV4Css({
			opts,
			runtimeState,
			runtime,
		...
		const outputCss = frameworkCssPipelineStrategy?.transformGeneratedCss?.(finalizedCss, {
			...cssPipelineContext,
			defaultWebCssCompat: (css) => (0, _weapp_tailwindcss_postcss.transformWebCssCompat)(css, resolveViteWebCssCompatOptions(cssPipelineContext)),
			removeScopedPreflight: removeScopedTailwindPreflightCss,
			shouldApplyWebCssCompat
		}) ?? removeScopedTailwindPreflightCss(shouldApplyWebCssCompat ? (0, _weapp_tailwindcss_postcss.transformWebCssCompat)(finalizedCss, resolveViteWebCssCompatOptions(cssPipelineContext)) : finalizedCss);
		if (transient) {
			for (const dependency of generated.dependencies) hookContext?.addWatchFile?.(dependency);
			return outputCss;
		}
		const tracedCss = require_v4_generation_core.annotateCssSourceTrace(outputCss, {
			opts,
			tokenSources: require_v4_generation_core.createCssTokenSourceMap(getSourceCandidateSourcesForEntries(void 0), opts)
		});
		for (const dependency of generated.dependencies) hookContext?.addWatchFile?.(dependency);
...
${tracedCss}`;
	...
	const generateBundleContext = {
		...
		extractSourceCandidates: (file, source) => require_v4_generation_core.extractCandidatesFromSource(source, node_path.default.extname((0, _weapp_tailwindcss_shared.cleanUrl)(file)).slice(1) || "html", {
			bareArbitraryValues: opts.arbitraryValues?.bareArbitraryValues,
			customAttributesEntities,
			disabledDefaultTemplateHandler
		}),
		...
		getOriginalCssLayerSource: (file) => originalCssLayerSourceByFile.get((0, _weapp_tailwindcss_shared.cleanUrl)(file)),
	...
	const installFrameworkWatchCssCacheAdapter = async (config) => {
		...
		if (wrapViteCssPostTransform(config, async (css, id) => {
			...
			if (require_v4_generation_core.hasBundlerGeneratedCssMarker(css)) {
...
function emitDeferredRuntimeLogs(opts, profile, rawOptions) {
	...
	require_compiler.logTailwindcssTarget(opts.tailwindRuntime, opts.tailwindcssBasedir);
	require_compiler.logRuntimeTailwindcssVersion(opts.tailwindcssBasedir, opts.tailwindRuntime?.packageInfo?.rootPath, opts.tailwindRuntime?.packageInfo?.version);
}
function createDispatcher(options) {
	...
	const ensureBranch = (config, context) => {
		...
		if (profile.appType && profile.appType !== initialProfile.appType && !options.appType && effectiveAppType === void 0) {
			...
			_weapp_tailwindcss_logger.logger.debug("根据 Vite 项目根目录自动推断 appType -> %s", profile.appType);
		}
